...
public class PinnedSectionListView extends ListView {
	...
	// fields used for drawing bg_shadow under a pinned section
	...
	/** bg_shadow instance with a pinned view, can be null. */
	...
	private final OnScrollListener mOnScrollListener = new OnScrollListener() {
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			...
			if (isFirstVisibleItemSection) {
				...
				if (sectionView.getTop() == getPaddingTop()) { // view sticks to the top, no need for pinned bg_shadow
					destroyPinnedShadow();
				} else { // section doesn't stick to the top, make sure we have a pinned bg_shadow
				...
				} else { // there is no section for the first visible item, destroy bg_shadow
					destroyPinnedShadow();
				}
			}
		};
...
//        initShadow(true);
	...
	/** Create bg_shadow wrapper with a pinned view for a view at given position */
	void createPinnedShadow(int position) {

		// try to recycle bg_shadow
		PinnedSection pinnedShadow = mRecycleSection;
		mRecycleSection = null;

		// create new bg_shadow, if needed
		...
		// initialize pinned bg_shadow
		pinnedShadow.view = pinnedView;
		pinnedShadow.position = position;
		pinnedShadow.id = getAdapter().getItemId(position);

		// store pinned bg_shadow
		mPinnedSection = pinnedShadow;
	}

	/** Destroy bg_shadow wrapper for currently pinned view */
	void destroyPinnedShadow() {
		if (mPinnedSection != null) {
			// keep bg_shadow for being recycled later
			mRecycleSection = mPinnedSection;
			mPinnedSection = null;
		}
	}

	/** Makes sure we have an actual pinned bg_shadow for given position. */
	void ensureShadowForPosition(int sectionPosition, int firstVisibleItem, int visibleItemCount) {
		if (visibleItemCount < 2) { // no need for creating bg_shadow at all, we have a single visible item
			destroyPinnedShadow();
			return;
		}

		if (mPinnedSection != null
				&& mPinnedSection.position != sectionPosition) { // invalidate bg_shadow, if required
			destroyPinnedShadow();
		}

		if (mPinnedSection == null) { // create bg_shadow, if empty
			createPinnedShadow(sectionPosition);
		}

		// align bg_shadow according to next section position, if needed
		...
		if (nextPosition < getCount()) {
			...
			if (nextSectionPosition > -1) {
				...
				if (mSectionsDistanceY < 0) {
					// next section overlaps pinned bg_shadow, move it up
	...
	public void setAdapter(ListAdapter adapter) {
		...
		// destroy pinned bg_shadow, if new adapter is not same as old one
